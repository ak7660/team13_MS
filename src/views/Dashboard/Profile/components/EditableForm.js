...
const EditableForm = ({ event }) => {
    ...
    const { eventName, location, numOfParticipant, quotaParticipant, eventDescription, dateTimeStart, dateTimeEnd, requiredTask, trainingMaterials } = event;

    const [tasks, setTasks] = useState(requiredTask.map(task => ({
        taskName: task.task,
        numOfVolunteers: task.quotaVolunteer,
        dateTimeStart: task.startTime,
        dateTimeEnd: task.endTime
    })));
    const [materials, setMaterials] = useState(trainingMaterials.map(material => ({
    ...
    return (
        <form>
            ...
            <FormControl id="description" isRequired paddingBottom={2}>
                ...
                <Textarea placeholder="Write description here" defaultValue={eventDescription}/>
            </FormControl>
            <FormControl id="date" isRequired paddingBottom={2}>
              <FormLabel>Date</FormLabel>
              <Flex gap={2}>
                <Input
                  placeholder="Start date" defaultValue={dateTimeStart}
                />
                <Input
                  placeholder="End date" defaultValue={dateTimeEnd}
                />
              </Flex>
            </FormControl>
          ...
          <Button onClick={addTask} colorScheme="yellow" size="sm" mt={4} textColor="white">
              Add Task
          </Button>
